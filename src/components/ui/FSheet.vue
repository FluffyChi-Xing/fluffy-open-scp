...
interface Props { label?: string; width?: string }
...
<template><Teleport to="body"><div class="f-sheet-overlay" :class="{ visible: open }" :aria-hidden="open ? undefined : 'true'" @mousedown.self="open = false"><aside class="f-sheet" :class="{ visible: open }" :style="props.width ? { width: props.width } : undefined" role="dialog" aria-modal="true" :aria-label="props.label"><slot /></aside></div></Teleport></template>
