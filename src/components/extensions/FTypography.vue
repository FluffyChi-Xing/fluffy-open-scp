...
const tag = computed(() =>
  props.header ? `h${props.header}` : props.paragraphy ? "p" : "span",
);
const classes = computed(() => [
  "f-typography",
  `f-typography-${props.type}`,
  `f-typography-${props.spacing}`,
  props.header && `f-typography-h${props.header}`,
]);
...
<template>
  <FTypographyParagraph
    v-if="props.paragraphy && props.ellipsis && !props.header"
...
.f-typography-secondary {
  color: var(--muted-foreground);
}
.f-typography-success {
  color: var(--success);
}
.f-typography-warning {
  color: var(--warning);
}
.f-typography-danger {
  color: var(--danger);
