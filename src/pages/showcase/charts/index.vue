...
const option = computed(() => ({
  ...
  xAxis: {
    ...
    axisLine: { lineStyle: { color: "#d8dae0" } },
  },
  ...
  series: [
    {
      ...
      type: "line",
      smooth: true,
      data: [84, 119, 92, 158, 141, 187, 206],
    },
    {
      name: "Deployments",
      type: "bar",
      barMaxWidth: 26,
      data: [12, 18, 15, 25, 21, 30, 35],
    },
  ],
}));
</script>
...
<template>
  <section class="page">
    <header>
      ...
      <h1>{{ $t("showcase.chartsTitle") }}</h1>
      <p>{{ $t("showcase.chartsDescription") }}</p>
    </header>
    <article class="panel">
      <div class="panel-heading">
        <div>
          <h2>{{ $t("showcase.traffic") }}</h2>
          <p>{{ $t("showcase.trafficDescription") }}</p>
        </div>
        <span>{{ $t("showcase.lastSevenDays") }}</span>
      </div>
      <FChart
        :modules="modules"
        :option="option"
