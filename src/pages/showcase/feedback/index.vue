...
<template>
  <section class="page">
    <header>
      ...
      <h1>{{ $t("showcase.feedbackTitle") }}</h1>
      <p>{{ $t("showcase.feedbackDescription") }}</p>
    </header>
    <div class="feedback-grid">
      <FPanel
        ><h2>{{ $t("feedback.toast") }}</h2>
      ...
      ><FPanel
        ><h2>{{ $t("feedback.loading") }}</h2>
        <div class="loading-demo">
          <FSpinner v-if="loading" :label="$t('common.loading')" /><span>{{
          ...
          ><FButton
            ...
            :loading="loading"
            @click="simulateLoad"
            >{{ $t("common.retry") }}</FButton
          >
        </div></FPanel
      ><FPanel
        ><div class="panel-heading">
          <h2>{{ $t("feedback.skeleton") }}</h2>
          <FButton variant="ghost" @click="showSkeleton = !showSkeleton">{{
            $t("feedback.toggle")
          }}</FButton>
        </div>
        <div v-if="showSkeleton" class="skeleton-stack">
          <FSkeleton width="42%" height="12px" /><FSkeleton
            height="16px"
          /><FSkeleton width="76%" height="16px" /><FSkeleton
            width="110px"
            height="34px"
          />
        </div>
        <p v-else class="loaded-copy">{{ $t("feedback.loaded") }}</p></FPanel
      >
    </div>
  </section>
</template>
