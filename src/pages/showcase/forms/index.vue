...
type ProfileForm = Record<"name" | "email" | "role" | "bio", string> &
  Record<"subscribe", boolean>;
const { t } = useI18n();
const { success } = useToast();
const manualName = shallowRef("Fluffy");
const manualEmail = shallowRef("hello@fluffy.dev");
const autoProfile = shallowRef<ProfileForm>({
  name: "",
  email: "",
  role: "developer",
  bio: "",
...
const columns: FormColumn<Record<string, string | boolean>>[] = [
  ...
  },
  {
    field: "bio",
    labelKey: "form.bio",
    placeholderKey: "form.bioPlaceholder",
    type: "textarea",
    span: 2,
  },
...
<template>
  <section class="page">
    <header>
      ...
      <h1>{{ $t("showcase.formsTitle") }}</h1>
      <p>{{ $t("showcase.formsDescription") }}</p>
    </header>
    <div class="form-grid">
      <FPanel
        ><h2>{{ $t("showcase.manualForm") }}</h2>
        <form class="manual-form" @submit.prevent="submitManual">
          <FFormItem id="manual-name" :label="$t('form.name')"
            ><template #default="field"
              ><FInput
                ...
                " /></template></FFormItem
          ><FFormItem id="manual-email" :label="$t('form.email')"
            ><template #default="field"
              ><FInput
                :id="field.id"
                v-model="manualEmail"
                ...
                " /></template></FFormItem
          ><FFormItem id="manual-bio" :label="$t('form.bio')"
            ><template #default="field"
              ><FTextarea
                :id="field.id"
                :placeholder="
                  $t('form.bioPlaceholder')
                " /></template></FFormItem
          ><FButton type="submit">{{ $t("form.submit") }}</FButton>
        </form></FPanel
      ><FPanel
        ><h2>{{ $t("showcase.autoForm") }}</h2>
        <FForm v-model="autoProfile" :columns="columns" @submit="submitAuto"
          ><template #field-subscribe="{ column }"
            ><label class="checkbox-line"
              ><FCheckbox v-model="autoProfile.subscribe" />{{
                $t(column.labelKey)
              }}</label
            ></template
          ></FForm
