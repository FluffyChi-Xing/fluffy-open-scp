...
<template>
  <main class="not-found">
    ...
    <article class="not-found-card">
      <p class="hero" aria-hidden="true">
        <span class="digit">4</span><span class="digit digit-accent">0</span
        ><span class="digit">4</span>
      </p>
      <h1>{{ $t("notFound.title") }}</h1>
      <p class="description">{{ $t("notFound.description") }}</p>
      <div class="missing-path">
        <span class="label">{{ $t("notFound.path") }}</span
        ><code>{{ route.fullPath }}</code>
      </div>
      <div class="actions">
        <RouterLink to="/" class="link"
          ><FButton>{{ $t("notFound.action") }}</FButton></RouterLink
        ><FButton v-if="canGoBack" variant="secondary" @click="router.back()">{{
          $t("notFound.back")
        }}</FButton>
      </div>
    </article>
...
.hero {
  font-size: clamp(4.25rem, 13vw, 6.5rem);
  font-weight: 800;
