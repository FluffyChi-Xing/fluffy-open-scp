...
import SetupGuardDialog from "@/components/setup/SetupGuardDialog.vue";
...
const currentPage = computed(() =>
  t(String(route.meta.titleKey ?? "navigation.home")),
);
const sidebarClass = computed(() => ({
  sidebar: true,
  "sidebar-collapsed": app.isSidebarCollapsed,
  "mobile-open": isMobileNavOpen.value,
}));
const appShellStyle = computed(() => ({
  "--menu-width": `${app.menuWidth}px`,
}));

function routeTab(): AppTab {
  return {
    fullPath: route.fullPath,
    titleKey: String(route.meta.titleKey ?? "navigation.home"),
    closable: !route.meta.noAffix,
  };
}
function syncTabs() {
  tabs.openTab(routeTab());
}
...
function closeLeftTabs(path: string) {
  tabs.closeLeftTabs(path);
  router.push(path);
}
function closeRightTabs(path: string) {
  tabs.closeRightTabs(path);
  router.push(path);
}
...
function closeAllTabs() {
  const nextPath = tabs.closeAllTabs();
  if (nextPath) router.push(nextPath);
}
...
function syncMobileState(event: MediaQueryListEvent | MediaQueryList) {
  ...
  if (!event.matches) isMobileNavOpen.value = false;
}
...
watch([isMobileNavOpen, isSearchOpen], ([navOpen, searchOpen]) => {
...
watch(
  () => app.colorWeak,
  (value) => {
    document.documentElement.classList.toggle("color-weak", value);
  },
  { immediate: true },
);
</script>
<template>
  <div
    class="app-shell"
    :class="{ 'no-navbar': !app.showNavbar }"
    :style="appShellStyle"
  >
    <Navbar
      v-if="app.showNavbar"
      ...
      :collapsed="app.isSidebarCollapsed"
      :dark="app.isDark"
      :locale="app.locale"
      :header-actions="appConfig.headerActions"
      @toggle-sidebar="toggleNavigation"
      @toggle-theme="app.toggleTheme"
      @toggle-locale="app.toggleLocale"
      @open-search="isSearchOpen = true"
      @open-settings="isSettingsOpen = true"
    /><CommandPalette
      :open="isSearchOpen"
      :items="leafNavigationItems"
    ...
    <div class="shell-body">
      ...
      >
        <SidebarNav
          ...
          :top-level-items="topLevelNavigationItems"
          @navigate="closeMobileNavigation"
          @open-external="openExternal"
        />
        <div class="sidebar-footer">
          <span class="status-dot" aria-hidden="true" />{{
            $t("shell.allSystemsOperational")
          }}
        </div>
      </aside>
      <main class="page-content">
        ...
        <div class="content-frame">
          <RouterView v-slot="{ Component }"
            ><component
              :is="Component"
              :key="`${route.fullPath}:${viewRevision}`"
          /></RouterView>
        </div>
        <footer class="footer">
          <span>{{ appConfig.name }}</span
          ><span>{{ $t("shell.footerStatus") }}</span>
        </footer>
      </main>
    </div>
    <FSheet v-model:open="isSettingsOpen" :label="$t('panel.title')"
      ><SettingsPanel /></FSheet
    ><SetupGuardDialog /><ChatAssistantDock />
  </div>
</template>
<style scoped>
.app-shell {
...
.content-frame {
  margin-inline: auto;
  max-width: 100%;
  padding: 36px clamp(20px, 4vw, 56px) 0;
  width: 100%;
}
.footer {
  color: var(--subtle-foreground);
  ...
  margin-top: auto;
  max-width: 1180px;
  padding: 40px clamp(20px, 4vw, 56px) 20px;
  width: 100%;
}
.drawer-overlay {
