...
**tile 世界尺寸判定 [中]**：拼图宽高比 19:17 ≈ 1.118 与地块表坐标跨度比
13472m/12128m ≈ 1.111 吻合（区域地形恰好覆盖地块分布范围），得
**tile ≈ 1024 m（cell ≈ 4 m）**；约束条件 = 土壤画刷位于世界坐标 -12146 m
（mosaic 必须覆盖之，排除 8 m/格方案）。交叉验证待引擎常量（§12）。
...
（DAT_00e12968 = tolower）。包内 tile 的 instance 即该 name-hash（名内含
tile 坐标 x/y 与 mip——穷举 0..63×0..63 未命中，坐标域/格式细节待续）。
