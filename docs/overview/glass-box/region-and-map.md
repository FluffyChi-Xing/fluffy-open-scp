...
垂直比例 **已破解**（2026-09-23，见 §12.1）：HeightMap 为 UNORM u16，terrain VS
解码 `height *= 2048; height -= 1024`，即 **z(米) = raw/32 − 1024**（raw 0 →
-1024 m，65535 → +1024 m）；水平方向：地形格边长 8 m（terrain.md §2.6，
...
- **朝向**：F0 = identity（拼合图与游戏区域视图同向）。2026-09-23 用户
  指出此前区域判定有误并给出官方图鉴校准点，已用 **白水谷（BEAF0510，
  5 城 + 1 伟工）** 复核：Y 形汇合河流 + 河岸 5 地块 + 环山结构与官方
  缩略图/描述完全同构，identity 朝向成立；"镜像感"来自官方缩略图的
  45° 等距投影（菱形摆拍）与俯视投影的差异。**ED 相对帧仍待定**。
- **区域身份与命名机制**（2026-09-23）：区域 desc 内的数字字符串
  （0x4EE97F5B，如 BC357A2B="257367589"、9F735B20="277412358"）= 区域
  UI id，且 **组 id = FNV1(数字串)**（BC357A2B 实证）；显示名在
  Locale/zh-tw/Data.package（type 0x0A98EAF0，JSON：locale hash→文本，
  白水谷=0x77cb42ea），locale key 的哈希输入未破。
- **区域 ↔ 地块表 ↔ 城市组显式配对**（region_bind 探针）：地块表
  （instance 2B9C480C）**每区域一份、同 instance 不同 group**，正确选表
  = 城市单条组 id 交集（城市组 36B property 显式引用区域 desc 全 TGI）；
  RT0 规律：**每区域 = N 城地块（表）+ 1 伟大工程位（多出的单条组）**
  ——BEAF0510（5+1）即白水谷；9F735B20（14 城）即绵延不毛之地
  （用户确认荒漠无森林，图鉴 14 城 + 2 伟工）。
...
- 运行时高度公式（`fStack_28 = DAT_0103d444 * 0.5 + DAT_0103d448`）的常量在
  BSS/未转储区——但垂直映射已从 shader 源码容器直接破解（见 §12.1：
  z = raw/32 − 1024，`height *= 2048; height -= 1024`）。
...
**伟大工程位坐标不在静态包内**（2026-09-23）：区域 desc 中两个悬空 Key
（0xEDB0E3AE、0x2687C033，各区域指向不存在的实例）疑似伟工元数据的
在线引用；Game 包全量扫描零引用。伟工位坐标来自服务器元数据/存档侧，
预览以 `--gw <wx> <wy>` 手动标注（白水谷实测 ≈ 世界 (2656,2496)，
右下角山上平地，与游戏内位置一致）。
...
**游戏上色机制**（模型，2026-09-23 二次修正）：渲染层不做"资源着色"——
按 ED 材质权重对层栈模板（ground/soil/water，贴图 + 色调 (0.48,0.39,0.27)，
SimCity_Game 包 D7EF2862 族）做 per-cell 纹理 splat（cMaterialGround /
cGroundTextureSet / cMaterialGroundDataViewBlendColors），森林密度驱动
树实例，水面 = Tessendorf 全局水位平面；外加光照。

**水位面与配色实证（2026-09-23 引擎实证修正，白水谷 BEAF0510 校准）**：

0. **水位面 = -870 m（raw 4928），此前 L≈3336 为河床刻平高度误读，已撤回**。
   证据链：① shader 源码容器（0x0469A3F7）terrain VS 垂直解码
   `height *= 2048; height -= 1024` → z = raw/32 − 1024；② `SimCity.exe`
   全二进制唯一浮点对 `-870.0, 1.4`（0x9A36E4，math 常量页），1.4 =
   HLSL `GetBeachTreeLine()`（`seaLevel = mWaterLevel.x + 1.4` 树线剔除）；
   ③ 区域 desc 0x00FEAAB3=1024.0（高度半幅）与 0x0E16BE1A=-870.0（水面），
   全部区域组同值；④ 地平线群岛（D01FA985）按 4928 渲染后仅四座大岛出露，
   与官方图鉴完全一致（3336 曾致全域海岸侵蚀状噪声出露）。

1. **b1 ≠ 水**：identity 帧下 b1 均值平原 121.9 ≫ 河床 52.6 ≫ 山地 34.2
   ——b1 是**草地/植被密度**（平原草盛、山地岩裸、水面无草）；
2. **~~水位面 = 高度直方图大跳变~~（推断路线已废弃）**：直方图跳变桶
   （2816-3327）实为**设计者河床刻平高度**（水位下方的雕刻底面），不是水面
   本身——水面应由引擎常量给出（见上条，-870 m = raw 4928）；环中位数法在
   全零框区域失效（已废弃）；
3. 修正后 `region_color` 渲染与官方图鉴结构一致：Y 形汇流河 + 河岸 5
   地块 + 草地/岩石分带；"镜像感"系官方缩略图 45° 等距摆拍所致，
   俯视 identity 朝向与其拓扑一致；
4. **草地判定 = 湿度(b1) × 平缓度**（用户对照官方图鉴纠错后修正）：
   山上平坦高地同为绿地（b1 高 + 坡度低），陡坡露岩——非海拔分带；
   早先"rel>2500 强制岩石"的硬编码已移除。
...
| 引擎 | 垂直比例常量（已破解：z = raw/32 − 1024，shader 容器实证 §12.1） | EXE + shader 0x0469A3F7 | 二进制级（已解析） |
...
- [x] ~~**垂直比例**（u16 高度 → 米）~~ **已破解**（2026-09-23，无需脱壳）：
  shader 源码容器（type 0x0469A3F7，SimCity_App.package）terrain VS 直接给出
  解码式——HeightMap 按 UNORM u16 采样后 `height *= 2048.0; height -= 1024.0;`
  即 **z(米) = raw/32 − 1024**（raw 0=-1024 m，raw 65535=+1024 m，1 raw =
  1/32 m）；城市 world 为 2048×2048 m（`controlMapUV = worldPosition.xy/2048+0.5`）；
- [x] ~~**全局水位面**~~ **已定案 -870 m（raw 4928）**（§5 证据链）；旧值 3336
  系河床刻平高度误读，已撤回；水面渲染应按引擎语义为平面 + 深浅渐变，
  不透射海底地形光照；
- [x] ~~**ED 金字塔网格按区域求解**~~ **已废弃——ED 槽位全游戏共享**
  （2026-09-23 `ed_grid_cross` 实证）：ED 341 槽位与 F0 完全独立（交集 0），
  但跨 11 个区域组逐槽一致（差异 0/256）；现以 `SHARED_ED_GRID` 常量内置
  （BEAF0510 求得、3 区域独立求解复核）。按区域贪心求解的失败根因：ED 场
  极平滑（最优误差中位 0.13），贪心在低误差海中选出**成环伪树**→无根→None
  （B12DE348 实证：341/341 子块误差全部 ≤60 仍失败）；
...
- Tauri 命令：`map_panel_list_regions` / `map_panel_render_region`（v1.1 起
  无状态化——ED 网格确认为全游戏共享后移除按区域求解缓存，渲染 200ms 级）；
...
- 区域显示名：已确认映射内置（白水谷=BEAF0510、绵延不毛之地=9F735B20、
  地平线群岛=D01FA985——11 城 + 3 伟工与官方一致），其余待 locale 哈希破解。

**v1.1（2026-09-23 三次更新）**：

- **水位面修正 3336 → 4928**（引擎实证，§5/§12.1）；
- **ED 网格内置共享常量** `SHARED_ED_GRID`，B12DE348「金字塔解码失败」修复；
- 水陆判定改用 4×4 box 滤波高度（≈引擎 mip 降采样），水面平整着色——
  消除 mip0 雕刻噪声碎岛与海底地形透射（地平线群岛「海岸侵蚀」根因）；
- 面板 UI：全屏 sheet 关闭按钮对齐 property editor 的 outline 样式；滚轮缩放
  改为以鼠标指针为锚点；展开按钮移入预览卡片顶部工具条（不再悬浮遮挡
  标尺与比例徽标）。
